...
impl DirExt for cap_std::fs::Dir {
    ...
    #[inline]
    fn open_dir_nofollow<P: AsRef<Path>>(&self, path: P) -> io::Result<Self> {
        open_dir_nofollow(&self.as_file_view(), path.as_ref())
            .map(|file| unsafe { Self::from_std_file(file) })
    }
...
impl AsyncDirExt for cap_async_std::fs::Dir {
    ...
    async fn set_atime<P: AsRef<async_std::path::Path> + Send>(
        ...
        spawn_blocking(move || set_times(&clone.as_file_view(), path.as_ref(), Some(atime), None))
            .await
    }
    ...
    async fn set_mtime<P: AsRef<async_std::path::Path> + Send>(
        ...
        let path = path.as_ref().to_path_buf();
        let clone = self.clone();
        spawn_blocking(move || set_times(&clone.as_file_view(), path.as_ref(), None, Some(mtime)))
            .await
    }
    ...
    async fn set_times<P: AsRef<async_std::path::Path> + Send>(
        ...
        let path = path.as_ref().to_path_buf();
        let clone = self.clone();
        spawn_blocking(move || set_times(&clone.as_file_view(), path.as_ref(), atime, mtime)).await
    }
    ...
    async fn set_symlink_times<P: AsRef<async_std::path::Path> + Send>(
        ...
        let clone = self.clone();
        spawn_blocking(move || {
            set_times_nofollow(&clone.as_file_view(), path.as_ref(), atime, mtime)
        })
        .await
    }
    ...
    >(
        ...
        if self.metadata(&src).await?.is_dir() {
            spawn_blocking(move || symlink_dir(src.as_ref(), &clone.as_file_view(), dst.as_ref()))
                .await
        } else {
    ...
    async fn open_dir_nofollow<P: AsRef<async_std::path::Path> + Send>(
        ...
        let clone = self.clone();
        spawn_blocking(move || {
            open_dir_nofollow(&clone.as_file_view(), path.as_ref())
                .map(|file| unsafe { Self::from_std_file(file.into()) })
        })
        .await
    }
...
impl DirExtUtf8 for cap_std::fs_utf8::Dir {
    ...
    #[inline]
    fn open_dir_nofollow<P: AsRef<str>>(&self, path: P) -> io::Result<Self> {
        open_dir_nofollow(&self.as_file_view(), path.as_ref().as_ref())
            .map(|file| unsafe { Self::from_std_file(file.into()) })
    }
...
impl AsyncDirExtUtf8 for cap_async_std::fs_utf8::Dir {
    ...
    async fn set_atime<P: AsRef<str> + Send>(
        ...
        let path = from_utf8(path)?;
        let clone = self.clone();
        spawn_blocking(move || set_times(&clone.as_file_view(), &path, Some(atime), None)).await
    }
    ...
    async fn set_mtime<P: AsRef<str> + Send>(
        ...
        let path = from_utf8(path)?;
        let clone = self.clone();
        spawn_blocking(move || set_times(&clone.as_file_view(), &path, None, Some(mtime))).await
    }
    ...
    async fn set_times<P: AsRef<str> + Send>(
        ...
        let path = from_utf8(path)?;
        let clone = self.clone();
        spawn_blocking(move || set_times(&clone.as_file_view(), &path, atime, mtime)).await
    }
    ...
    async fn set_symlink_times<P: AsRef<str> + Send>(
        ...
        let path = from_utf8(path)?;
        let clone = self.clone();
        spawn_blocking(move || set_times_nofollow(&clone.as_file_view(), &path, atime, mtime)).await
    }
    ...
    async fn symlink<P: AsRef<str> + Send, Q: AsRef<str> + Send>(
        ...
        let clone = self.clone();
        spawn_blocking(move || symlink(&src, &clone.as_file_view(), &dst)).await
    }
    ...
    async fn symlink<P: AsRef<str> + Send, Q: AsRef<str> + Send>(
        ...
        let metadata =
            spawn_blocking(move || stat(&clone.as_file_view(), &src_, FollowSymlinks::Yes)).await?;
        let clone = self.clone();
        if metadata.is_dir() {
            spawn_blocking(move || symlink_dir(&src, &clone.as_file_view(), &dst)).await
        } else {
            spawn_blocking(move || symlink_file(&src, &clone.as_file_view(), &dst)).await
        }
    }
    ...
    async fn open_dir_nofollow<P: AsRef<str> + Send>(&self, path: P) -> io::Result<Self> {
        let path = from_utf8(path)?;
        let clone = self.clone();
        spawn_blocking(move || open_dir_nofollow(&clone.as_file_view(), path.as_ref()))
            .await
            .map(|file| unsafe { Self::from_std_file(file.into()) })
    }
